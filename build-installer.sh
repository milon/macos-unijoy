...
if [[ ! -f "$KEYLAYOUT_FILE" ]]; then
    echo "Error: unijoy.keylayout not found at $KEYLAYOUT_FILE" >&2
    exit 1
fi
if [[ ! -f "$ICON_FILE" ]]; then
    echo "Error: unijoy.icns not found at $ICON_FILE" >&2
    exit 1
fi
...
ditto --norsrc --noextattr --noqtn "$KEYLAYOUT_FILE" "$PAYLOAD_DIR/Library/Keyboard Layouts/unijoy.keylayout"
ditto --norsrc --noextattr --noqtn "$ICON_FILE"      "$PAYLOAD_DIR/Library/Keyboard Layouts/unijoy.icns"
...
chmod 644 "$PAYLOAD_DIR/Library/Keyboard Layouts/unijoy.keylayout"
chmod 644 "$PAYLOAD_DIR/Library/Keyboard Layouts/unijoy.icns"
